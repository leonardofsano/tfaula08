...
export default function ProductsTwoCols() {
    ...
    const handleDelete = async (id: number) => {
        // optimistic: call API then remove from state
        const res = await deleteProductApi(id);
        if ((res as any).error) return alert('Falha ao excluir o produto.');
    ...
    const handleCreate = (newProduct: any) => {
        // Add newly created product to the top of the list
        setData((prev) => {
            if (!prev || prev === "error") return prev;
            return { ...prev, rows: [newProduct, ...prev.rows] };
        });
    }
