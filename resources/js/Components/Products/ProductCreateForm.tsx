...
export default function ProductCreateForm({ onCreate }: ProductCreateFormProps) {
    ...
    const onCreateHandler = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        setErrorMsg(null);
        const priceNumber = parsePriceToThousand(price);
        if (!name) return setErrorMsg("Nome é obrigatório.");
        if (priceNumber === null) return setErrorMsg("Preço inválido.");
        ...
        (async () => {
            try {
                const resp = await fetch('/api/products', {
                    ...
                    body: JSON.stringify({ name, price_times_thousand: priceNumber })
                });
                if (!resp.ok) {
                    const j = await resp.json().catch(() => ({}));
                    return setErrorMsg((j && j.message) || 'Erro ao criar produto');
                }
